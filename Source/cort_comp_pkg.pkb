...
*/
  ...
  gc_inline_constraint       CONSTANT VARCHAR2(30) := 'CONSTRAINT';
...
--  gc_create_index            CONSTANT VARCHAR2(30) := '<create_index>';
  ...
  FUNCTION comp_column_arr(
    ...
    in_source_column_arr IN arrays.gt_str_arr,
    in_target_column_arr IN arrays.gt_str_arr,
...
*/        
  ...
  FUNCTION get_from_clause(
    in_join_inner_owner_arr IN arrays.gt_str_arr,      -- Array of owner of join inner table
    in_join_inner_table_arr IN arrays.gt_str_arr,      -- Array of name of join inner table
    in_join_outer_owner_arr IN arrays.gt_str_arr,      -- Array of owner of join inner table
    in_join_outer_table_arr IN arrays.gt_str_arr       -- Array of name of join inner table
  )
  RETURN VARCHAR2
  AS
  ...
  BEGIN
    ...
    FOR i IN 1..in_join_inner_table_arr.COUNT LOOP
      l_full_name := '"'||in_join_inner_owner_arr(i)||'"."'||in_join_inner_table_arr(i)||'"';
      IF NOT l_owner_table_ind.EXISTS(l_full_name) THEN
        l_result := l_result || l_full_name ||',';
        l_owner_table_ind(l_full_name) := i;
      END IF;
      l_full_name := '"'||in_join_outer_owner_arr(i)||'"."'||in_join_outer_table_arr(i)||'"';
      IF NOT l_owner_table_ind.EXISTS(l_full_name) THEN
        l_result := l_result || l_full_name ||',';
        l_owner_table_ind(l_full_name) := i;
      END IF;
  ...
  FUNCTION get_where_clause(
    in_join_inner_owner_arr  IN arrays.gt_str_arr,      -- Array of owner of join inner table
    in_join_inner_table_arr  IN arrays.gt_str_arr,      -- Array of name of join inner table
    in_join_inner_column_arr IN arrays.gt_str_arr,      -- Array of column of join inner table
    in_join_outer_owner_arr  IN arrays.gt_str_arr,      -- Array of owner of join inner table
    in_join_outer_table_arr  IN arrays.gt_str_arr,      -- Array of name of join inner table
    in_join_outer_column_arr IN arrays.gt_str_arr       -- Array of column of join inner table
  )
  RETURN VARCHAR2
  AS
  ...
  BEGIN
    ...
    FOR i IN 1..in_join_inner_column_arr.COUNT LOOP
      l_full_name := '"'||in_join_inner_owner_arr(i)||'"."'||
                          in_join_inner_table_arr(i)||'"."'||
    ...
    IF in_column_rec.data_type_owner IS NULL THEN
      CASE 
       WHEN in_column_rec.data_type IN ('CHAR', 'VARCHAR2') THEN
         RETURN get_clause(in_column_rec.data_type, in_column_rec.data_length||' '||get_clause(gc_char_used,in_column_rec.char_used), '(', ')');
       WHEN in_column_rec.data_type IN ('NCHAR', 'NVARCHAR2') THEN
         RETURN get_clause(in_column_rec.data_type, in_column_rec.char_col_decl_length, '(', ')');
       WHEN in_column_rec.data_type = 'NUMBER' AND in_column_rec.data_precision IS NOT NULL THEN
         RETURN get_clause(in_column_rec.data_type, in_column_rec.data_precision||','||in_column_rec.data_scale, '(', ')');
  ...
  AS
    l_clause  CLOB;
  BEGIN
    IF NOT cort_exec_pkg.g_params.physical_attr THEN
  ...
  FUNCTION get_physical_attr_clause(
  ...
  BEGIN
    IF NOT cort_exec_pkg.g_params.physical_attr THEN
      l_clause := get_clause(gc_pctfree, in_physical_attr_rec.pct_free)||
                  get_clause(gc_pctused, in_physical_attr_rec.pct_used)||       
                  get_clause(gc_initrans, in_physical_attr_rec.ini_trans)||
                  get_storage_clause(in_physical_attr_rec.storage);
    END IF;              
    RETURN l_clause;
  END get_physical_attr_clause;
  ...
  BEGIN
    l_result := gc_result_nochange;
    IF cort_exec_pkg.g_params.physical_attr THEN
    ...
    WHEN gc_result_recreate THEN
      debug('Compare physical_attr.sorage - 1');
      RETURN gc_result_recreate;
  ...
  BEGIN
    ...
    IF comp_value(in_source_physical_attr_rec.pct_free, in_target_physical_attr_rec.pct_free) = 1 THEN
      debug('Compare index physical_attr - 1');
      RETURN gc_result_recreate; 
    END IF;            
    IF comp_value(in_source_physical_attr_rec.ini_trans, in_target_physical_attr_rec.ini_trans) = 1 THEN
      l_result := gc_result_alter; 
    ...
    CASE l_comp_result
    WHEN gc_result_recreate THEN 
      debug('Compare index physical_attr.sorage - 1');
      RETURN gc_result_recreate;
    WHEN gc_result_alter THEN 
      l_result := gc_result_alter; 
      l_frwd_clause := l_frwd_clause || get_clause(gc_storage, l_storage_frwd_clause, '(', ')');
  ...
  BEGIN
    ...
    FOR i IN 1..in_subpart_templ_arr.COUNT LOOP
      ...
      ELSE 
        NULL;      
      END CASE;
      IF in_subpart_templ_arr(i).tablespace_name IS NOT NULL THEN
      ...
      FOR j IN 1..in_subpart_templ_arr(i).lob_template_arr.COUNT LOOP
        ...
        IF l_lob_template_rec.tablespace_name IS NOT NULL THEN
  ...
  AS
    ...
    l_frwd_clause            CLOB;
    l_rlbk_clause            CLOB;
    l_parallel_clause        VARCHAR2(30);
    l_result                 PLS_INTEGER;
    l_move_needed            BOOLEAN;
    l_comp_result            PLS_INTEGER; 
  BEGIN
    l_result := gc_result_nochange;
    ...
    IF comp_value(in_source_table_rec.iot_type, in_target_table_rec.iot_type) = 1 THEN
      ...
      RETURN gc_result_recreate;
    END IF; 
    IF comp_value(in_source_table_rec.overflow_table_name, in_target_table_rec.overflow_table_name) = 1 THEN
      IF cort_exec_pkg.g_params.revertable OR 
         in_target_table_rec.overflow_table_name IS NULL
      THEN    
...
--    END IF; 
    ...
    l_move_needed := cort_exec_pkg.g_params.force_move;
    ...
    -- Move or Alter
    -- Logging
    IF comp_value(in_source_table_rec.logging, in_target_table_rec.logging) = 1 
    THEN
      l_result := gc_result_alter;
    ...
    CASE l_comp_result 
    WHEN gc_result_recreate THEN
      debug('Compare physical_attr_rec - 1');
      RETURN gc_result_recreate; 
    WHEN gc_result_alter THEN
      l_result := gc_result_alter;
    ...
    THEN
      ...
      l_rlbk_clause_indx_arr(gc_key_compression) := get_clause(gc_key_compression, in_source_table_rec.iot_key_compression, NULL, ' '||in_target_table_rec.iot_prefix_length);
    END IF;  
    ...
    IF in_target_table_rec.overflow_table_name IS NOT NULL THEN
      ...
      -- overflow logging 
      IF comp_value(in_source_table_rec.overflow_logging, in_target_table_rec.overflow_logging) = 1 
      THEN
        l_result := gc_result_alter;
        l_frwd_clause_indx_arr(gc_overflow) := gc_overflow;
        l_rlbk_clause_indx_arr(gc_overflow) := gc_overflow;   
      ...
      CASE l_comp_result 
      WHEN gc_result_recreate THEN
        debug('Compare overflow physical_attr_rec - 1');
        RETURN gc_result_recreate; 
      WHEN gc_result_alter THEN
        l_result := gc_result_alter;
        l_frwd_clause_indx_arr(gc_overflow) := gc_overflow;
    ...
    IF l_result = gc_result_alter AND l_move_needed THEN
      l_frwd_clause := get_clause_by_name(l_frwd_clause_indx_arr, gc_tablespace)||
      ...
      l_rlbk_clause := get_clause_by_name(l_rlbk_clause_indx_arr, gc_tablespace)||
    ...
    ELSIF l_result = gc_result_alter AND NOT l_move_needed THEN
    ...
    IF in_source_table_rec.partitioned = 'YES' THEN
      IF comp_value(in_source_table_rec.interval, in_target_table_rec.interval) = 1 THEN
        l_result := gc_result_alter;
        l_frwd_clause := get_clause(gc_interval, in_target_table_rec.interval, '(', ')');
        l_rlbk_clause := get_clause(gc_interval, in_source_table_rec.interval, '(', ')');
        add_alter_table_stmt(
          io_frwd_stmt_arr => io_frwd_alter_stmt_arr, 
          io_rlbk_stmt_arr => io_rlbk_alter_stmt_arr, 
          in_table_name    => in_source_table_rec.table_name, 
  ...
  BEGIN
    ...
    IF in_lob_rec.securefile = 'YES' THEN 
      l_clause_indx_arr(gc_retention) := get_clause(gc_retention, in_lob_rec.retention, NULL, in_lob_rec.minretention);
    END IF; 
  ...
  -- Drop column. Applicable only for virtual columns. For segment column set unused is used/
  ...
  BEGIN
    ...
    l_rlbk_clause := gc_add_column||' '||get_add_column_clause(in_source_table_rec, in_column_rec);
  ...
  AS
    ...
    l_rlbk_clause            CLOB;
  BEGIN
    l_frwd_clause := get_clause(gc_set_unused_column, in_column_rec.column_name, '"', '"');
    l_rlbk_clause := gc_add_column||' '||get_add_column_clause(in_source_table_rec, in_column_rec);
    add_alter_table_stmt(
      io_frwd_stmt_arr => io_frwd_alter_stmt_arr, 
      io_rlbk_stmt_arr => io_rlbk_alter_stmt_arr, 
  ...
  END set_unused_column;
  
  -- Find column in string array
  FUNCTION find_column(
    in_column_name_arr IN arrays.gt_str_arr,
  ...
  AS
    l_full_join_index_name VARCHAR2(100);
    l_indx                 PLS_INTEGER;
  BEGIN
    FOR i IN 1..io_source_table_rec.constraint_arr.COUNT LOOP
      IF find_column(io_source_table_rec.constraint_arr(i).column_arr, in_column_rec.column_name) THEN
        io_source_table_rec.constraint_arr(i).drop_flag := TRUE;
      END IF; 
    END LOOP;
    FOR i IN 1..io_source_table_rec.index_arr.COUNT LOOP
      IF find_column(io_source_table_rec.index_arr(i).column_arr, in_column_rec.column_name) THEN
        io_source_table_rec.index_arr(i).drop_flag := TRUE;
      END IF; 
    END LOOP;
    FOR i IN 1..io_source_table_rec.log_group_arr.COUNT LOOP
      IF find_column(io_source_table_rec.log_group_arr(i).column_arr, in_column_rec.column_name) THEN
        io_source_table_rec.index_arr(i).drop_flag := TRUE;
      END IF; 
    END LOOP;
    IF in_column_rec.join_index_column THEN
      FOR i IN 1..in_column_rec.join_index_arr.COUNT LOOP
        l_full_join_index_name := in_column_rec.join_index_arr(i); 
        IF io_source_table_rec.index_indx_arr.EXISTS(l_full_join_index_name) THEN
          l_indx := io_source_table_rec.index_indx_arr(l_full_join_index_name);
          io_source_table_rec.index_arr(l_indx).drop_flag := TRUE;
        END IF;
        IF io_source_table_rec.join_index_indx_arr.EXISTS(l_full_join_index_name) THEN
          l_indx := io_source_table_rec.join_index_indx_arr(l_full_join_index_name);
          io_source_table_rec.join_index_arr(l_indx).drop_flag := TRUE;
        END IF;
      END LOOP;
    END IF;
  ...
  AS
    ...
    l_cort_val               VARCHAR2(4000);
    l_frwd_clause            CLOB;
    l_rlbk_clause            CLOB;
    l_check_clause           VARCHAR2(32767);
    l_frwd_clause_indx_arr   arrays.gt_xlstr_indx;
    l_rlbk_clause_indx_arr   arrays.gt_xlstr_indx;
    l_skip_comp_data_length  BOOLEAN := FALSE;
    l_force_check_default    BOOLEAN := FALSE;
    l_revert_flag            BOOLEAN := TRUE;
    l_full_join_index_name   VARCHAR2(65);
    l_indx                   PLS_INTEGER;
  BEGIN
    l_result := gc_result_nochange;
    IF --comp_value(in_source_column_rec.column_id, in_target_column_rec.column_id) = 1 OR
       comp_value(in_source_column_rec.segment_column_id, in_target_column_rec.segment_column_id) = 1 OR
       comp_value(in_source_column_rec.virtual_column, in_target_column_rec.virtual_column) = 1 OR
       comp_value(in_source_column_rec.hidden_column, in_target_column_rec.hidden_column) = 1
    THEN
    ...
    ELSE
      ...
      IF comp_value(in_source_column_rec.data_type, in_target_column_rec.data_type) = 1 OR
        ...
        IF in_target_column_rec.cort_value_indx IS NOT NULL AND  -- there is cort-value 
        ...
        ELSE
         ...
         IF (in_source_column_rec.data_type = 'CHAR' AND in_target_column_rec.data_type = 'VARCHAR2' AND
             ...
             in_source_column_rec.data_length <= in_target_column_rec.data_length) OR
            (in_source_column_rec.data_type = 'NCHAR' AND in_target_column_rec.data_type = 'NVARCHAR2' AND
             -- applicable only if we do not decrease length 
             in_source_column_rec.data_length <= in_target_column_rec.data_length)
          THEN
            -- non-revertable
      ...
      IF (NOT l_skip_comp_data_length) AND 
         (comp_value(in_source_column_rec.data_length, in_target_column_rec.data_length) = 1 OR
          ...
          comp_value(in_source_column_rec.data_scale, in_target_column_rec.data_scale) = 1)
      THEN 
        IF in_target_column_rec.cort_value_indx IS NOT NULL THEN 
    ...
    THEN
      ...
      IF in_source_column_rec.new_column_name = in_target_column_rec.column_name OR
         -- it is virtual column 
         in_target_column_rec.virtual_column = 'YES' 
      THEN
        -- then column could be renamed
        IF in_source_column_rec.join_index_column THEN
          FOR i IN 1..in_source_column_rec.join_index_arr.COUNT LOOP
            l_full_join_index_name := in_source_column_rec.join_index_arr(i); 
            IF io_source_table_rec.index_indx_arr.EXISTS(l_full_join_index_name) THEN
              l_indx := io_source_table_rec.index_indx_arr(l_full_join_index_name);
              io_source_table_rec.index_arr(l_indx).recreate_flag := TRUE;
            END IF;
            IF io_source_table_rec.join_index_indx_arr.EXISTS(l_full_join_index_name) THEN
              l_indx := io_source_table_rec.join_index_indx_arr(l_full_join_index_name);
              io_source_table_rec.join_index_arr(l_indx).recreate_flag := TRUE;
            END IF;
          END LOOP;
        END IF;
  ...
  AS
    ...
    l_source_virt_col_id_arr  arrays.gt_int_arr;
    l_target_virt_col_id_arr  arrays.gt_int_arr;
    l_source_segm_col_id_arr  arrays.gt_int_arr;
    l_target_segm_col_id_arr  arrays.gt_int_arr;
    ...
    l_dummy_frwd_stmt_arr     arrays.gt_clob_arr;
    l_dummy_rlbk_stmt_arr     arrays.gt_clob_arr;
    l_dummy_check_stmt_arr    arrays.gt_xlstr_arr;
  ...
  BEGIN
    ...
    -- Copy all indexes of segment columns and virtual columns into separate collections
    -- for source table
    FOR i IN 1..io_source_table_rec.column_arr.COUNT LOOP
      ...
      THEN    
        l_source_segment_col_arr(i) := i;
      END IF;  
      -- get virtual non-hidden columns.
      IF io_source_table_rec.column_arr(i).virtual_column = 'YES' AND
         io_source_table_rec.column_arr(i).hidden_column = 'NO' 
      THEN    
        l_source_virt_col_id_arr(i) := io_target_table_rec.column_arr(i).column_id;
      END IF;  
    END LOOP;
    ...
    FOR i IN 1..io_target_table_rec.column_arr.COUNT LOOP
      ...
      THEN    
        l_target_segment_col_arr(i) := i;
        l_add_column_arr(i) := i;
      END IF;  
      -- get virtual non-hidden columns.
      IF io_target_table_rec.column_arr(i).virtual_column = 'YES' AND
         io_target_table_rec.column_arr(i).hidden_column = 'NO' 
      THEN    
        l_target_virt_col_id_arr(i) := io_target_table_rec.column_arr(i).column_id;
      END IF;  
    END LOOP;
    ...
    WHILE l_indx1 IS NOT NULL LOOP
      ...
      IF io_target_table_rec.column_indx_arr.EXISTS(l_column_name) THEN
        ...
        l_source_segm_col_id_arr(l_indx1) := io_source_table_rec.column_arr(l_indx1).column_id; 
        l_target_segm_col_id_arr(l_indx2) := io_target_table_rec.column_arr(l_indx2).column_id;
    ...
    -- [Smart rename]
    -- check that removed columns have added on the same position column with the same data type 
    IF l_drop_column_arr.COUNT > 0 THEN
      l_indx1 := l_drop_column_arr.FIRST;
      WHILE l_indx1 IS NOT NULL LOOP
        -- if new column is being added on the same position  
        IF l_add_column_arr.EXISTS(l_indx1) AND
           io_target_table_rec.column_arr(l_indx1).cort_value_indx IS NULL AND
           comp_data_type(
             in_source_column_rec => io_source_table_rec.column_arr(l_indx1), 
             in_target_column_rec => io_target_table_rec.column_arr(l_indx1)
           ) = 0 AND
           comp_value(
             in_val1 => io_source_table_rec.column_arr(l_indx1).data_default, 
             in_val2 => io_target_table_rec.column_arr(l_indx1).data_default
            ) = 0
        THEN
          ...
          io_source_table_rec.column_arr(l_indx1).new_column_name := io_target_table_rec.column_arr(l_indx1).column_name;
          ...
          l_source_segm_col_id_arr(l_indx1) := io_source_table_rec.column_arr(l_indx1).column_id; 
          l_target_segm_col_id_arr(l_indx1) := io_target_table_rec.column_arr(l_indx1).column_id;
          io_source_table_rec.column_arr(l_indx1).matched_column_id := io_target_table_rec.column_arr(l_indx1).column_id; 
          io_target_table_rec.column_arr(l_indx1).matched_column_id := io_source_table_rec.column_arr(l_indx1).column_id; 
          l_drop_column_arr.DELETE(l_indx1);
          l_add_column_arr.DELETE(l_indx1);
    ...
    IF l_drop_column_arr.COUNT > 0 THEN
      -- and rollback mode is enabled
      IF cort_exec_pkg.g_params.revertable THEN
        -- then need to recreate
        debug('There are dropped segment columns');
        RETURN gc_result_recreate;
      ELSE
        l_cnt := 0;
        l_indx1 := l_source_segment_col_arr.FIRST;
        WHILE l_indx1 IS NOT NULL LOOP
          IF l_drop_column_arr.EXISTS(l_indx1) THEN
           -- remove dropping columns from source segment column array
            ...
            l_cnt := l_cnt + 1;
          ELSE
            -- decrease segment_id by number of dropped columns for every existing segment column
            ...
            IF io_target_table_rec.column_indx_arr.EXISTS(l_column_name) THEN
              ...
              IF comp_value(io_source_table_rec.column_arr(l_indx1).segment_column_id, io_target_table_rec.column_arr(l_indx2).segment_column_id) = 1 
              THEN
    ...
    THEN
      ...
      l_indx2 := l_target_segm_col_id_arr.NEXT(l_indx2);
      ...
      IF l_indx2 IS NOT NULL THEN
        debug('There are new segment columns in the middle of the column list');
        RETURN gc_result_recreate;
      ELSE
        l_segment_result := gc_result_alter;
      END IF;
    ...
    -- update column_id for remained source columns 
    l_cnt := 1;
    FOR i IN 1..io_source_table_rec.column_arr.COUNT LOOP
      IF l_source_segm_col_id_arr.EXISTS(i) THEN
        l_source_segm_col_id_arr(i) := l_cnt;
        l_cnt := l_cnt + 1;
      ELSIF l_source_virt_col_id_arr.EXISTS(i) THEN
        l_source_virt_col_id_arr(i) := l_cnt;
        l_cnt := l_cnt + 1;
      END IF;
    END LOOP;

    -- check all for matched segment columns
    l_indx1 := l_source_segm_col_id_arr.FIRST;
    l_indx2 := l_target_segm_col_id_arr.FIRST;
    WHILE l_indx1 IS NOT NULL AND l_indx2 IS NOT NULL LOOP
      debug('compare columns: '||io_source_table_rec.column_arr(l_indx1).column_name||' and '||io_target_table_rec.column_arr(l_indx2).column_name);
    ...
    l_indx1 := l_source_virt_col_id_arr.FIRST;
    l_indx2 := l_target_virt_col_id_arr.FIRST;
    l_cnt := 0; 
    WHILE l_indx1 IS NOT NULL AND l_indx2 IS NOT NULL LOOP
      l_source_column_id := l_source_virt_col_id_arr(l_indx1);
      l_target_column_id := l_target_virt_col_id_arr(l_indx2);
      
      IF l_source_column_id - l_cnt < l_target_column_id THEN
        ...
        END IF;
        l_cnt := l_cnt + 1;
        l_indx1 := l_source_virt_col_id_arr.NEXT(l_indx1);
      ELSIF l_source_column_id - l_cnt = l_target_column_id THEN
        ...
        -- go to next pair
        l_indx1 := l_source_virt_col_id_arr.NEXT(l_indx1);
        l_indx2 := l_target_virt_col_id_arr.NEXT(l_indx2);
      ELSE  
    ...
    IF l_virtual_result < gc_result_exchange THEN
      -- if there are any left source virtual columns - drop them 
      WHILE l_indx1 IS NOT NULL LOOP
        l_drop_column_arr(l_indx1) := l_indx1;
        IF l_first_old_virt_col_indx IS NULL THEN
          l_first_old_virt_col_indx := l_indx1;
    ...
    -- if there are dropped virtual columns in the middle of the list and revertable mode is enabled
    IF l_first_old_virt_col_indx IS NOT NULL 
    THEN
      -- get the first dropped virtual column
      l_indx1 := l_first_old_virt_col_indx;
      l_indx1 := l_source_segm_col_id_arr.NEXT(l_indx1);
      IF l_indx1 IS NOT NULL AND cort_exec_pkg.g_params.revertable  
      THEN
        -- then need to recreate
        l_virtual_result := gc_result_exchange;
      ELSE
    ...
    IF l_virtual_result = gc_result_exchange AND 
       NOT cort_exec_pkg.g_params.keep_data
    THEN
    ...
    WHILE l_indx1 IS NOT NULL LOOP
      IF io_source_table_rec.column_arr(l_indx1).virtual_column = 'YES' THEN
        ...
        IF (io_source_table_rec.column_arr(l_indx1).partition_key) OR 
           (io_source_table_rec.iot_type IS NOT NULL AND find_column(io_source_table_rec.iot_pk_column_arr, io_source_table_rec.column_arr(l_indx1).column_name)) 
    ...
    WHILE l_indx2 IS NOT NULL LOOP
      ...
      THEN
        IF io_target_table_rec.column_arr(l_indx2).cort_value_indx IS NOT NULL THEN
  ...
  BEGIN
    FOR i IN 1..in_index_rec.column_arr.COUNT LOOP
      l_result := l_result || NVL(in_index_rec.column_expr_arr(i), in_index_rec.column_arr(i))||',';  
    END LOOP;
    l_result := TRIM(',' FROM l_result);
    RETURN l_result; 
  ...
  BEGIN
    ...
    l_clause_indx_arr(gc_on_table) := get_clause(gc_on_table, in_index_rec.table_owner||'"."'||in_index_rec.table_name, '"', '"');
    ...
    END IF;
    -- key_compression
    l_clause_indx_arr(gc_key_compression) := get_clause(gc_key_compression, in_index_rec.compression, NULL, ' '||in_index_rec.prefix_length);
    -- locality
  ...
  BEGIN
    -- create forward statements
    l_frwd_stmt := get_create_index_clause(in_index_rec.rename_rec.object_name, in_index_rec);
    -- create rollback statements
  ...
  BEGIN
    ...
    -- create rollback statements
    l_rlbk_stmt := get_create_index_clause(in_index_rec.index_name, in_index_rec);
    add_stmt(
      io_frwd_stmt_arr => io_frwd_alter_stmt_arr, 
      io_rlbk_stmt_arr => io_rlbk_alter_stmt_arr, 
      in_frwd_stmt     => l_frwd_stmt,
  ...
  BEGIN
    ...
    THEN
      debug('Recreate index : '||io_source_index_rec.index_name||' - mismatch index_type/itype/uniqueness/locality/compression/tablespace');
      RETURN gc_result_recreate;
    END IF; 
    ...
    IF in_comp_phys_attr THEN
      ...
      CASE l_comp_result
      WHEN gc_result_recreate THEN
        debug('Compare index physical_attr_rec - 1');
        RETURN gc_result_recreate;
      WHEN gc_result_alter THEN
        l_result := gc_result_alter;
        l_frwd_clause_indx_arr(gc_physical_attr) := l_frwd_clause;
  ...
  AS
    ...
    l_comp_result             PLS_INTEGER;
    l_alter_frwd_stmt_arr     arrays.gt_clob_arr;
    l_alter_rlbk_stmt_arr     arrays.gt_clob_arr;
    l_drop_frwd_stmt_arr      arrays.gt_clob_arr;
    l_drop_rlbk_stmt_arr      arrays.gt_clob_arr;
    l_create_frwd_stmt_arr    arrays.gt_clob_arr;
    l_create_rlbk_stmt_arr    arrays.gt_clob_arr;
  BEGIN
    -- build for source indexes description indx-array 
    FOR i IN 1..in_source_table_rec.index_arr.COUNT LOOP
      IF in_source_table_rec.index_arr(i).constraint_name IS NULL THEN
    ...
    FOR i IN 1..in_target_table_rec.index_arr.COUNT LOOP
      IF in_target_table_rec.index_arr(i).constraint_name IS NULL THEN
    ...
    WHILE l_hash_index IS NOT NULL LOOP
      ...
      IF l_target_hash_index_arr.EXISTS(l_hash_index) THEN
        ...
        l_comp_result := comp_index(
                           ...
                           io_frwd_alter_stmt_arr => l_alter_frwd_stmt_arr,
                           io_rlbk_alter_stmt_arr => l_alter_rlbk_stmt_arr
                         );
        -- if need to recreate                                    
        IF l_comp_result = cort_comp_pkg.gc_result_recreate THEN
          cort_comp_pkg.drop_index(
            in_index_rec           => l_source_index_rec,
            io_frwd_alter_stmt_arr => l_drop_frwd_stmt_arr,
            io_rlbk_alter_stmt_arr => l_drop_rlbk_stmt_arr
          );
          cort_comp_pkg.create_index(
            in_index_rec           => l_target_index_rec,
            io_frwd_alter_stmt_arr => l_create_frwd_stmt_arr,
            io_rlbk_alter_stmt_arr => l_create_rlbk_stmt_arr
          );
        END IF;
      ELSE
    ...
    -- Add all drop statements first 
    FOR i IN 1..l_drop_frwd_stmt_arr.COUNT LOOP
      io_frwd_stmt_arr(io_frwd_stmt_arr.COUNT + 1) := l_drop_frwd_stmt_arr(i); 
      io_rlbk_stmt_arr(io_rlbk_stmt_arr.COUNT + 1) := l_drop_rlbk_stmt_arr(i); 
    END LOOP;

    -- Add all alter  statements first 
    FOR i IN 1..l_alter_frwd_stmt_arr.COUNT LOOP
      io_frwd_stmt_arr(io_frwd_stmt_arr.COUNT + 1) := l_alter_frwd_stmt_arr(i); 
      io_rlbk_stmt_arr(io_rlbk_stmt_arr.COUNT + 1) := l_alter_rlbk_stmt_arr(i); 
    END LOOP;

    -- Add all create statements next 
    FOR i IN 1..l_create_frwd_stmt_arr.COUNT LOOP
      io_frwd_stmt_arr(io_frwd_stmt_arr.COUNT + 1) := l_create_frwd_stmt_arr(i); 
      io_rlbk_stmt_arr(io_rlbk_stmt_arr.COUNT + 1) := l_create_rlbk_stmt_arr(i); 
    END LOOP;
  END comp_indexes;
  ...
  AS
    ...
    l_old_column_arr          arrays.gt_str_arr;
    l_indx                    VARCHAR2(30);
  BEGIN
    ...
    ELSE
      -- drop all references
      IF cort_exec_pkg.g_params.keep_refs AND cort_exec_pkg.g_params.keep_bad_refs THEN
        cort_exec_pkg.raise_error('Unable to preserve references on changing primary/unique key');
      ELSE
    ...
    FOR i IN 1..io_ref_contraint_arr.COUNT LOOP
      IF NOT cort_exec_pkg.g_params.validate_refs THEN
      ...
      IF NOT io_source_table_rec.constraint_arr(l_source_hash_cons_arr(l_hash_index)).drop_flag THEN
        IF l_target_hash_cons_arr.EXISTS(l_hash_index) THEN
          ...
          IF l_cons_result = gc_result_recreate THEN
            ...
            IF l_index_result >= gc_result_recreate THEN
              IF l_source_constraint_rec.index_name IS NOT NULL THEN
                drop_index(
                  in_index_rec           => l_source_index_rec,
                  io_frwd_alter_stmt_arr => io_frwd_alter_stmt_arr,
                  io_rlbk_alter_stmt_arr => io_rlbk_alter_stmt_arr
            ...
            IF l_source_constraint_rec.constraint_type IN ('P','U') AND
               ...
               cort_exec_pkg.g_params.keep_refs 
            THEN
              restore_references(
        ...
        ELSE
          ...
          THEN
            IF cort_exec_pkg.g_params.keep_refs AND cort_exec_pkg.g_params.keep_bad_refs THEN
              cort_exec_pkg.raise_error('Unable to preserve references on droping primary/unique key');
            END IF;
  ...
  BEGIN
    FOR i IN 1..io_source_table_rec.ref_constraint_arr.COUNT LOOP
      l_constraint_rec := io_source_table_rec.ref_constraint_arr(i);
      l_constraint_rec.validated := 'NOT VALIDATED';
      l_frwd_clause := get_clause(gc_drop_constraint, l_constraint_rec.rename_rec.current_name, '"', '"');
      l_rlbk_clause := get_add_constraint_clause(l_constraint_rec, NULL);
      add_alter_table_stmt(
        io_frwd_stmt_arr => io_frwd_alter_stmt_arr, 
        io_rlbk_stmt_arr => io_rlbk_alter_stmt_arr, 
        in_table_name    => l_constraint_rec.table_name, 
        in_owner         => l_constraint_rec.owner, 
        in_frwd_clause   => l_frwd_clause,
        in_rlbk_clause   => l_rlbk_clause
      );
    END LOOP;  
  END drop_cort_references;
  ...
  FUNCTION comp_lob_params(
    ...
    out_frwd_lob_parames OUT CLOB,
    out_rlbk_lob_parames OUT CLOB
  )
  RETURN NUMBER
  AS
    l_result                 PLS_INTEGER;
  ...
  BEGIN
    ...
    IF (comp_value(in_source_lob_rec.retention, in_target_lob_rec.retention) = 1 OR 
       comp_value(in_source_lob_rec.minretention, in_target_lob_rec.minretention) = 1) AND
       in_target_lob_rec.securefile = 'YES'  
    THEN 
      l_result := gc_result_alter;
      l_frwd_clause_indx_arr(gc_retention) := get_clause(gc_retention, in_target_lob_rec.retention, NULL, in_target_lob_rec.minretention);
      l_rlbk_clause_indx_arr(gc_retention) := get_clause(gc_retention, in_source_lob_rec.retention, NULL, in_source_lob_rec.minretention);
    END IF; 
    ...
    IF l_result = gc_result_alter AND cort_exec_pkg.g_params.force_move THEN
      l_result := gc_result_alter_move;
    END IF;
  ...
  BEGIN
    ...
    IF l_result = gc_result_alter_move THEN
      ...
      l_frwd_clause := gc_move||                              
                       ...
                       get_clause(gc_parallel, cort_exec_pkg.g_params.parallel);
  ...
  BEGIN
    -- segment attr:
    IF in_partition_rec.partition_type <> 'HASH' THEN
      -- physical attrs
      l_clause_indx_arr(gc_physical_attr) := get_physical_attr_clause(in_partition_rec.physical_attr_rec);
      -- Logging
    ...
    IF in_partition_rec.subpartition_from_indx > 0 AND 
       in_partition_rec.subpartition_to_indx > 0 
    THEN
  ...
  BEGIN
    ...
    io_target_partition_rec.matching_indx := io_source_partition_rec.indx;
    io_source_partition_rec.matching_indx := io_target_partition_rec.indx;
    ...
    IF io_source_partition_rec.partition_level = 'PARTITION' THEN
      ...
      CASE l_comp_result
      WHEN gc_result_recreate THEN
        debug('Compare partition physical_attr_rec - 1');
        RETURN gc_result_recreate;
      WHEN gc_result_alter THEN
        l_result := gc_result_alter;
        l_frwd_clause_indx_arr(gc_physical_attr) := l_frwd_clause;
    ...
    -- Tablespace
    IF comp_value(io_source_partition_rec.tablespace_name, io_target_partition_rec.tablespace_name) = 1 THEN
      l_result := gc_result_alter;
      l_move := TRUE;
    ...
    IF io_source_table_rec.overflow_table_name IS NOT NULL THEN
      ...
      -- Tablespace
      IF comp_value(io_source_partition_rec.overflow_tablespace, io_target_partition_rec.overflow_tablespace) = 1 THEN
        l_result := gc_result_alter;
        l_move := TRUE;
    ...
    IF io_source_partition_rec.partition_level = 'PARTITION' AND
       io_source_table_rec.subpartitioning_type <> 'NONE' THEN
      l_result := comp_subpartitions(
                    io_source_table_rec     => io_source_table_rec,
    ...
    ELSIF io_source_partition_arr.COUNT < io_target_partition_arr.COUNT THEN
      FOR i IN io_source_partition_arr.COUNT+1..io_target_partition_arr.COUNT LOOP
        ...
        WHEN 'SUBPARTITION' THEN
          l_frwd_clause := get_clause(gc_modify_partition, io_target_partition_arr(i).parent_partition_name, '"', '"')||
                           get_clause(gc_add_subpartition, io_target_partition_arr(i).partition_name, '"', '"')||
                           get_partition_desc_clause(io_target_partition_arr(i), empty_partition_arr);
          l_rlbk_clause := get_clause(gc_drop_subpartition, io_target_partition_arr(i).partition_name, '"', '"');
        END CASE; 
        add_alter_table_stmt(
          io_frwd_stmt_arr => io_frwd_alter_stmt_arr, 
          io_rlbk_stmt_arr => io_rlbk_alter_stmt_arr, 
  ...
  BEGIN
    ...
    FOR i IN 1..io_target_partition_arr.COUNT LOOP 
      ...
      IF l_source_cnt <= l_source_partition_arr.COUNT THEN
        l_part_result := partition_utils.compare_high_values(
                           in_values1        => l_source_partition_arr(l_source_cnt).high_values,
        ...
        IF l_part_result = partition_utils.compare_equal THEN
          l_part_result := comp_partition(
                             io_source_table_rec     => io_source_table_rec,
                             io_target_table_rec     => io_target_table_rec,
                             io_source_partition_rec => io_source_partition_arr(l_source_cnt), 
                             io_target_partition_rec => io_target_partition_arr(i), 
                             io_frwd_alter_stmt_arr  => io_frwd_alter_stmt_arr,
                             io_rlbk_alter_stmt_arr  => io_rlbk_alter_stmt_arr
                           );
          ...
          IF l_result = gc_result_recreate THEN
            RETURN l_result;
          END IF;
          l_source_cnt := l_source_cnt + 1;
        ELSE
          -- not match. search through other partitions 
          l_part_rec := partition_utils.find_list_part_by_highvalue(
                          in_partition_arr  => l_source_partition_arr,
                          in_values         => l_target_partition_arr(i).high_values
                        );
          -- if exists but on another position
          IF l_part_rec.partition_name IS NOT NULL THEN
            -- need to recreate table
            RETURN gc_result_recreate;
          ELSE
            -- need to add (insert on given position)
            l_result := gc_result_alter;
            CASE io_target_partition_arr(i).partition_level
            WHEN 'PARTITION' THEN
              partition_utils.insert_list_partition_ddl(
                ...
                in_partition_desc   => get_partition_desc_clause(io_target_partition_arr(i), io_target_table_rec.subpartition_arr),
                in_before_partition => l_source_partition_arr(l_source_cnt).partition_name,
    ...
    FOR i IN 1..io_target_partition_arr.COUNT LOOP 
      ...
      IF l_source_cnt <= l_source_partition_arr.COUNT THEN
        ...
        IF l_part_result = partition_utils.compare_equal THEN
          ...
          l_result := GREATEST(l_result, l_part_result);
          IF l_result = gc_result_recreate THEN
            debug('drop partition '||l_source_partition_arr(l_source_cnt).partition_name);
            RETURN l_result;
          END IF;
          debug('comp partition '||l_source_partition_arr(l_source_cnt).partition_name||' result = '||l_result);
          l_source_cnt := l_source_cnt + 1;
        ELSIF l_part_result = partition_utils.compare_greater THEN 
          -- add new target partition (insert on given position)
          debug('insert partition '||l_target_partition_arr(i).partition_name);
          l_result := gc_result_alter;
  ...
  BEGIN
    l_result := gc_result_nochange;
    IF io_source_table_rec.subpartitioning_type = 'NONE' THEN
      RETURN l_result;
    END IF;
    l_index := 1;
  ...
  BEGIN
    ...
    IF in_table_rec.partitioned = 'NO'
    THEN
      -- prepare partition clause 
      IF dbms_db_version.version >= 11 THEN
        -- use system partitioning
        l_partition_sql := 'PARTITION BY SYSTEM (PARTITION P_DEF)';
      ELSE                                   
        -- use list partitioning
        l_partition_sql := 'PARTITION BY LIST("'||in_swap_table_rec.part_key_column_arr(1)||'") (PARTITION P_DEF VALUES (DEFAULT))';
      END IF;  
    ELSIF in_table_rec.partitioned = 'YES' AND in_table_rec.subpartitioning_type = 'NONE'
    THEN
      IF is_subpartitioning_available(in_table_rec) THEN
        -- use list partitioning
        l_part_rec.table_owner        := in_table_rec.owner;
        l_part_rec.table_name         := in_table_rec.table_name;
        l_part_rec.partition_level    := 'PARTITION';
        l_part_rec.partition_type     := 'LIST';
        l_part_rec.partition_name     := 'P_DEF';
        l_part_rec.composite          := 'YES';
        l_part_rec.high_value         := 'DEFAULT';
        l_part_rec.position           := 1;
        l_part_rec.physical_attr_rec  := in_table_rec.partition_arr(1).physical_attr_rec;
        l_part_rec.subpartition_from_indx := 1;
        l_part_rec.subpartition_to_indx   := in_table_rec.partition_arr.COUNT;
        l_part_arr(1) := l_part_rec;
         
        l_subpart_arr := in_table_rec.partition_arr;
         
        l_partition_sql := 'PARTITION BY LIST("'||in_swap_table_rec.part_key_column_arr(1)||'")'||CHR(10)||
        '  SUBPARTITION BY '||in_table_rec.partitioning_type||'('||convert_arr_to_str(in_table_rec.part_key_column_arr)||')  ('||CHR(10)||
        get_partitions_sql(l_part_arr, l_subpart_arr)||CHR(10)||
        ')';
      ELSE
        l_partition_sql := NULL;
      END IF;   
    ELSIF in_table_rec.subpartitioning_type <> 'NONE' THEN
      FOR i IN in_partition_rec.subpartition_from_indx..in_partition_rec.subpartition_to_indx LOOP
        l_part_rec := in_table_rec.subpartition_arr(i);
        l_part_rec.partition_level := 'PARTITION';
        l_part_arr(l_subpart_arr.COUNT+1) := l_part_rec;
      END LOOP;
      -- use subpartitions as partitions      
      l_partition_sql := 'PARTITION BY '||in_table_rec.subpartitioning_type||' ('||convert_arr_to_str(in_table_rec.subpart_key_column_arr)||')  ('||CHR(10)||
      get_partitions_sql(l_part_arr, l_subpart_arr)||CHR(10)||
      ')';
    END IF;
  ...
  BEGIN
    ...
    FOR i IN 1..in_table_rec.constraint_arr.COUNT LOOP
      l_constraint_rec := in_table_rec.constraint_arr(i);
      l_constraint_rec.table_name := in_table_rec.rename_rec.temp_name;
      l_constraint_rec.constraint_name := in_table_rec.constraint_arr(i).rename_rec.temp_name;
      l_constraint_rec.rename_rec.current_name := in_table_rec.constraint_arr(i).rename_rec.temp_name;
    ...
    THEN
      ...
      get_partitions_sql(in_table_rec.partition_arr, in_table_rec.subpartition_arr)||CHR(10)||
      ')';
    ELSIF in_table_rec.partitioned = 'YES' AND in_table_rec.subpartitioning_type <> 'NONE' THEN
      l_partition_sql := 'PARTITION BY '||in_table_rec.partitioning_type||'('||convert_arr_to_str(in_table_rec.part_key_column_arr)||')'||CHR(10)||
      '  SUBPARTITION BY '||in_table_rec.subpartitioning_type||'('||convert_arr_to_str(in_table_rec.subpart_key_column_arr)||')  ('||CHR(10)||
      get_partitions_sql(in_table_rec.partition_arr, in_table_rec.subpartition_arr)||CHR(10)||
      ')';
    END IF;
    
    -- create swap table
    l_create_sql := 'CREATE TABLE "'||in_table_rec.owner||'"."'||in_table_rec.rename_rec.temp_name||'" ('||l_columns_list||CHR(10)||')'||
                    l_col_properties||CHR(10)||l_iot_clause||l_partition_sql;    
    
    l_drop_sql := 'DROP TABLE "'||in_table_rec.owner||'"."'||in_table_rec.rename_rec.temp_name||'"';
  ...
  BEGIN
    l_sql := ' EXCHANGE PARTITION '||in_partition_name||' WITH TABLE "'||in_target_table_rec.owner||'"."'||in_target_table_rec.rename_rec.current_name||'" INCLUDING INDEXES WITHOUT VALIDATION';
    add_alter_table_stmt(
      io_frwd_stmt_arr => io_frwd_stmt_arr, 
      io_rlbk_stmt_arr => io_rlbk_stmt_arr, 
  ...
  BEGIN
    ...
    l_clause_indx_arr(gc_hierarchy_option) := get_clause(gc_hierarchy_option, in_privilege_rec.grantable);
  ...
  BEGIN
    FOR i IN 1..in_source_table_rec.policy_arr.COUNT LOOP
      ...
      l_col_option := in_source_table_rec.policy_arr(i).column_option;
  ...
  BEGIN
    DBMS_RLS.ADD_GROUPED_POLICY (
      ...
      policy_type           => '''||in_source_table_rec.policy_arr(i).policy_type||''',
      ...
      sec_relevant_cols_opt => '||l_col_option||'
    );
  END;';   
  ...
  BEGIN
    FOR i IN 1..in_target_table_rec.column_arr.COUNT LOOP
      ...
      THEN
        ...
        l_cort_indx := in_target_table_rec.column_arr(i).cort_value_indx;
        IF in_source_table_rec.column_indx_arr.EXISTS(in_target_table_rec.column_arr(i).column_name) THEN
          ...
          IF l_cort_indx IS NOT NULL AND
             (in_target_table_rec.column_arr(i).cort_values(l_cort_indx).force_value OR comp_data_type(in_source_table_rec.column_arr(l_column_indx), in_target_table_rec.column_arr(i)) = 1) 
          THEN
            -- update value            
            out_values_list := out_values_list||in_target_table_rec.column_arr(i).cort_values(l_cort_indx).expression||l_column_alias;
          ELSE
            -- convert data type
            out_values_list := out_values_list||get_type_convert_expression(in_source_table_rec.column_arr(l_column_indx), in_target_table_rec.column_arr(i), cort_exec_pkg.g_params.alias)||l_column_alias;
          END IF;
        ELSIF l_cort_indx IS NOT NULL
        THEN
          out_values_list := out_values_list||in_target_table_rec.column_arr(i).cort_values(l_cort_indx).expression||l_column_alias;
        ELSE  
  ...
  AS
  BEGIN
    RETURN 'ALTER TABLE "'||in_owner||'"."'||in_table_name||'" MOVE LOB ("'||in_from_name||'") STORE AS "'||in_to_name||'"';
  ...
  AS
  BEGIN
    RETURN 'ALTER TABLE "'||in_owner||'"."'||in_table_name||'" MOVE VARRAY "'||in_from_name||'" STORE AS LOB "'||in_to_name||'"';
  ...
  AS
  BEGIN
    IF in_from_name <> in_to_name THEN
      CASE in_object_type
      WHEN 'TABLE' THEN
        io_frwd_stmt_arr(io_frwd_stmt_arr.COUNT+1) := get_rename_table_ddl(in_owner, in_from_name, in_to_name);
        io_rlbk_stmt_arr(io_rlbk_stmt_arr.COUNT+1) := get_rename_table_ddl(in_owner, in_to_name, in_from_name);
      WHEN 'INDEX' THEN
        io_frwd_stmt_arr(io_frwd_stmt_arr.COUNT+1) := get_rename_index_ddl(in_owner, in_from_name, in_to_name);
        io_rlbk_stmt_arr(io_rlbk_stmt_arr.COUNT+1) := get_rename_index_ddl(in_owner, in_to_name, in_from_name);
      WHEN 'CONSTRAINT' THEN
        io_frwd_stmt_arr(io_frwd_stmt_arr.COUNT+1) := get_rename_constraint_ddl(in_owner, in_table_name, in_from_name, in_to_name);
        io_rlbk_stmt_arr(io_rlbk_stmt_arr.COUNT+1) := get_rename_constraint_ddl(in_owner, in_table_name, in_to_name, in_from_name);
      WHEN 'TRIGGER' THEN
        io_frwd_stmt_arr(io_frwd_stmt_arr.COUNT+1) := get_rename_trigger_ddl(in_owner, in_from_name, in_to_name);
        io_rlbk_stmt_arr(io_rlbk_stmt_arr.COUNT+1) := get_rename_trigger_ddl(in_owner, in_to_name, in_from_name);
      WHEN 'LOB' THEN
        io_frwd_stmt_arr(io_frwd_stmt_arr.COUNT+1) := get_rename_lob_ddl(in_owner, in_table_name, in_from_name, in_to_name);
        io_rlbk_stmt_arr(io_rlbk_stmt_arr.COUNT+1) := get_rename_lob_ddl(in_owner, in_table_name, in_to_name, in_from_name);
      WHEN 'VARRAY' THEN
        io_frwd_stmt_arr(io_frwd_stmt_arr.COUNT+1) := get_rename_varray_lob_ddl(in_owner, in_table_name, in_from_name, in_to_name);
        io_rlbk_stmt_arr(io_rlbk_stmt_arr.COUNT+1) := get_rename_varray_lob_ddl(in_owner, in_table_name, in_to_name, in_from_name);
      END CASE;
    END IF;
  END rename_object;
